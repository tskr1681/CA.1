...
public class ScorpionEnergyMinimizationStep implements PipelineStep<Candidate, Candidate> {
    ...
    /**
     * Should at the very least set conformer scores and set the conformer file location
    ...
    private static List<Double> getConformerScores(Path scorpionoutput) throws IOException {
        ...
        //Did the block we were in contain a score? If not, we append Double.NEGATIVE_INFINITY, to guarantee it won't be the best one
        ...
        for (String line : l) {
            ...
            if (line.startsWith(">  <TOTAL>") && !block_has_score) {
                ...
                if (!block_has_score) {
                    conformerScores.add(Double.NEGATIVE_INFINITY);
                }
