...
public class SimilaritySelector {
    public static List<List<Integer>> getVariedReactants(List<List<String>> reactants, Path variation_executable, Path rdkit_wrapper, Path pipeline_location, long seed) throws IOException {
        List<List<Integer>> out = new ArrayList<>();
        for (int i = 0; i < reactants.size(); i++) {
            File in_file = pipeline_location.resolve(i + ".smiles").toFile();
            File out_file = in_file.toPath().resolveSibling(i + "_out.txt").toFile();
            ...
            out.add(bufferedReader.lines().map(s -> Integer.parseInt(s.replace("\n", ""))).collect(Collectors.toList()));
        }
